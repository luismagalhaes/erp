...
public sealed class PaymentStorage(AppDbContext dbContext) : IPaymentStorage
{
    ...
    {
        var hash = await dbContext.Set<Payment>()
            .AsNoTracking()
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var settled = await dbContext.Set<PaymentLine>()
            .AsNoTracking()
            .Where(line => documentIds.Contains(line.OriginatingDocumentId))
            .Where(line => !dbContext.Set<PaymentStatusChange>()
    ...
    {
        await dbContext.Set<PaymentStatusChange>().AddAsync(statusChange, cancellationToken);
    }
}
