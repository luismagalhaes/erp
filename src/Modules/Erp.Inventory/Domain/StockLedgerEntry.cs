...
public sealed class StockLedgerEntry
{
    ...
    /// <summary>
    /// What this movement moved value at. A purchase brings its own; everything else is stamped by
    /// the balance that applied it, with the average that was there at the time.
    /// </summary>
    /// <remarks>
    /// Stamping it is what makes the ledger self-describing — read a row and you see what it did to
    /// the value, not merely to the quantity — and it is what lets a reversal undo exactly what the
    /// original did instead of what stock happens to be worth today.
    /// </remarks>
    ...
    /// <summary>
    /// Records the cost this movement moved value at. Set once: a movement that already knows what
    /// it cost — a purchase, or the reversal of one — keeps it, and everything else takes the
    /// average from the balance applying it.
    /// </summary>
    public void ValueAt(decimal unitCost) => UnitCost ??= unitCost;
    ...
    /// </summary>
    /// <param name="unitCost">
    /// What the goods cost, when the correction knows. Opening stock does — it is the only way a
    /// cost gets in for a company that starts using the system with a warehouse already full — and
    /// an ordinary count does not, so what it finds is worth the average of what was already there.
    /// </param>
