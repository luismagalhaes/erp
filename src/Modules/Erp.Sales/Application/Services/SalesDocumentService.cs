    ...
    IOptions<FiscalOptions> fiscalOptions) : ISalesDocumentService
{
    ...
    public async Task<InvoiceDetailDto> IssueAsync(CreateInvoiceRequest request, string? userId, CancellationToken cancellationToken = default)
    {
        ...
        // A self-billing series has document type "FT" like any other, so nothing above catches it.
        // Its numbers belong to the supplier's documents and to the "S" SAF-T; using it here would
        // put one of our own sales into that chain.
        if (series.SelfBilling)
            throw new ArgumentException(
                $"Series '{series.SeriesCode}' is for self-billing, so it cannot number our own sales.",
                nameof(request));
