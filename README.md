...
- **Contagem de tentativas de registo é por instância, não partilhada** — [`ISignUpAttemptTracker`](src/Identity/Erp.Identity.Infrastructure/Application/ISignUpAttemptTracker.cs), que decide quando `SignUp.razor` passa a exigir reCAPTCHA, vive só em memória (`ConcurrentDictionary`), ao contrário da contagem do `SignIn`, que lê o `LoginAudit` na base de dados. Com um único processo do `Erp.Identity` (o caso hoje) isto é indiferente; com mais do que uma instância atrás de um load balancer, cada uma conta as suas próprias submissões, o que dilui o limiar de 3 tentativas por IP.
