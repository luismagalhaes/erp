    ...
    IWarehouseService warehouseService) : ControllerBase
{
    ...
    [HttpGet("odata")]
    [Authorize(Policy = Policies.Read)]
    [ProducesResponseType<ODataCollection<GoodsReceiptListItemDto>>(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public ActionResult<ODataCollection<GoodsReceiptListItemDto>> Query(
        [FromQuery] Guid companyId,
        ODataQueryOptions<GoodsReceiptListItemDto> options)
    {
        if (companyId == Guid.Empty)
            return BadRequest(new { error = "companyId is required." });

        return Ok(ODataQueryExecutor.Execute(receiptService.Query(companyId), options));
    }
