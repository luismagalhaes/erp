...
/// </summary>
public sealed class StockApiClient(HttpClient http) : ApiClientBase(http)
{
    // --- Warehouses ---
    ...
    /// <summary>Server side inventory count listing, filtered, sorted and paged by the database.</summary>
    public Task<(IReadOnlyList<InventoryCountListItem> Items, int Count, string? Error)> QueryInventoryCountsAsync(
        Guid companyId, ODataQuery query, CancellationToken cancellationToken = default) =>
        GetODataAsync<InventoryCountListItem>($"api/inventory-counts/odata?companyId={companyId}{query.ToQueryString()}", cancellationToken);
