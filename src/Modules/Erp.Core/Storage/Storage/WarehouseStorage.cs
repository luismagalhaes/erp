...
public sealed class WarehouseStorage(ErpDbContext dbContext) : IWarehouseStorage
{
    ...
    public Task<bool> CodeExistsAsync(
        ...
        dbContext.Set<Warehouse>().AnyAsync(
    ...
    public Task<Warehouse?> GetDefaultAsync(Guid companyId, CancellationToken cancellationToken = default) =>
        dbContext.Set<Warehouse>().FirstOrDefaultAsync(x => x.CompanyId == companyId && x.IsDefault, cancellationToken);

    public async Task AddAsync(Warehouse warehouse, CancellationToken cancellationToken = default) =>
        await dbContext.Set<Warehouse>().AddAsync(warehouse, cancellationToken);
